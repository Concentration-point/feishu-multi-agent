...
> - `01_审核库/广告法禁用词.md` — 通用禁用词 & 广告法合规
> - `01_审核库/小红书平台规则.md` — 小红书字数、结构、语气规范
> - `01_审核库/抖音平台规则.md` — 抖音脚本格式、时长、话术规范

调 `search_knowledge`，按以下关键词检索 `01_审核库` 下的规则文件：
...
`list_content` 返回的内容行备注字段中可能含有系统自动生成的 `[预检]` 标记，例如：
`[预检] 命中禁用词：最（共2次）、第一（共1次）`

**预检结果是提示，不是结论**，存在以下已知误报场景，必须结合原文上下文独立判断：

| 误报场景 | 判断方法 |
|---|---|
| "第一"作为序号使用（"第一点是…第二点是…"） | 查原文，若仅用于列举结构、无任何比较或排名含义 → `banned_words` 通过 |
| "最"出现在引用客户原话或描述客观场景中 | 查原文，若非对产品效果的主观绝对化断言 → 酌情通过 |
| 词条出现在免责声明、注意事项等法律用语中 | 上下文为法律声明语境 → 不视为违规 |

> **核心原则**：禁用词违规的本质是"以绝对化表述夸大产品功效"，上下文不构成此语义时不应判定违规。
...
| 维度 | 检查内容 |
|------|----------|
| **banned_words** | 禁用词 & 合规检查：是否以绝对化表述夸大产品功效（须结合上下文，序号/引用/法律声明语境中的禁用词不视为违规） |
...
- `dimensions`：五个维度各自的 通过/不通过（必须全部填写：`banned_words` / `brand_tone` / `platform_spec` / `dept_style` / `fact_check`）
...
全部内容行审核完毕后：
1. 调 `update_status` → "排期中"（具体通过率阈值由系统决定，你只需如实提交审核结果）
...
1. ✅ 已检索 `knowledge/01_审核库/` 下的规则文件（search_knowledge + read_knowledge）
